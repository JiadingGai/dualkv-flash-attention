...
class TestFlashFwdDualkvAttention:
    ...
    @pytest.mark.xfail
    def test_compare_with_reference0(self, dtype, seqlen, context_seqlen, rotary_dim, device="cuda", use_dualkv_attention=True):
        # sanity check test
        # set seed
        ...
        out = flash_attn_with_kvcache(
            ...
            use_dualkv_attention=use_dualkv_attention,
        )
        ...
        print("===================== out vs out_dualkv_ref ====================")
        print(f"Output max diff: {(out - out_dualkv_ref).abs().max().item()}")
        print(f"Output mean diff: {(out - out_ref).abs().mean().item()}")
        print(torch.allclose(out, out_dualkv_ref, rtol=1e-08, atol=9.8e-04))
        # manuall inspect the max diff in out vs out_ref
        diff_dualkv = (out - out_dualkv_ref).abs()
        xxx = torch.argwhere(diff_dualkv >= diff_dualkv.max())
        # FIXME: ugly indexing tensor with another tensor (use gather?)
        print("out_dualkv_ref@max_diff = ", out_dualkv_ref[xxx[0][0], xxx[0][1], xxx[0][2], xxx[0][3]], ", coordinate = ", xxx[0])
    ...
    def test_compare_fa_with_dualkv(self, dtype, bs, seqlen, context_seqlen_max, decoded_seqlen_max, rotary_dim):
        ...
        dualkv_time, og_fa_time = self.template_timing_run(dtype=torch.float16,
                                                          device="cuda",
                                                          bs=bs,
                                                          seqlen=seqlen,
    ...
    def test2(self, rotary_dim=128):
        ...
        for context_seqlen in range(128, 2*16384+1, 128):
          ...
          for seqlen in range(1,135+1,3):
            ...
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            # ret_code = self.test0(dtype=torch.float16, seqlen=seqlen, context_seqlen=context_seqlen, use_dualkv_attention=use_dualkv_attention)
    ...
    def test_timing(self, dtype, bs, seqlen, context_seqlen_max, decoded_seqlen_max, rotary_dim):
        ...
        for context_low in range(1, context_seqlen_max + 1, 257):
            ...
            dualkv_time, og_fa_time, log_msg  = self.template_timing_run(dtype=dtype,
                                                              device="cuda",
                                                              bs=bs,
                                                              seqlen=seqlen,
                                                              context_seqlen_max=context_seqlen_max,
    ...
    def template_timing_run(self, dtype=torch.float16, device="cuda", bs=5, seqlen=7, context_seqlen_max=1536, decoded_seqlen_max=2904, rotary_dim=128, num_runs=1, cache_seqlens_context=1536, cache_seqlens_decoded=2904):
        ...
        for xxx in range(num_runs):
          ...
          end_time = time.time()
          dualkv_time_list.append(end_time - start_time)
        ...
        for xxx in range(num_runs):
            ...
            out_og, lse_og = flash_attn_with_kvcache(
                ...
                k_cache=Kcache, # will be muted
                v_cache=Vcache, # will be muted
                k_cache_decoded=None, # only needed by dualkv attn
                v_cache_decoded=None, # only needed by dualkv attn
                k_cache_context=None, # only needed by dualkv attn
                v_cache_context=None, # only needed by dualkv attn
                k=k,
                v=v,
                rotary_cos=cos,
                rotary_sin=sin,
