"""Test DualKV kernel correctness for hdim=256 (kBlockN=64).
...
# Context is SHARED (bs=1), decoded is per-batch
...
# Reference: expand context to match batch
...
def main():
    configs = [
        ...
        # hdim=256, various context lengths (boundary cases for kBlockN=64)
        (1,   5,   0, 256, 8, 8),
        (1,  16,   0, 256, 8, 8),
        (1,  63,   0, 256, 8, 8),
        (1,  64,   0, 256, 8, 8),
    ...
    for bs, ctx, dec, hdim, nh, nhk in configs:
        label = f"bs={bs:2d} ctx={ctx:4d} dec={dec:3d} hdim={hdim} h={nh}/{nhk}"
