"""Test DualKV kernel correctness at kBlockN boundaries for all hdims.
For each hdim, tests context lengths at kBlockN*i - 1, kBlockN*i, kBlockN*i + 1
to catch off-by-one errors in block iteration logic.
Also varies decoded lengths including 0, 1, and boundary values."""
...
def main():
    # kBlockN by hdim
    block_sizes = {64: 256, 128: 128, 192: 64, 256: 64}

    configs = []
    for hdim in [64, 128, 192, 256]:
        kbn = block_sizes[hdim]
        # Test at block boundaries: kBlockN*i +/- 1 for i=1,2,3
        boundary_ctxs = set()
        for i in range(1, 4):
            boundary = kbn * i
            if boundary > 512:
        ...
        boundary_ctxs.update([1, 2, 3, kbn // 2])

        for ctx in sorted(boundary_ctxs):
            # For each context length, test with dec=0 and dec>0
            for dec in [0, 1, kbn // 2]:
                configs.append((1, ctx, dec, hdim, 8, 8))
            # One GQA test at exact boundary
            if ctx == kbn:
                configs.append((2, ctx, kbn // 4, hdim, 8, 2))
