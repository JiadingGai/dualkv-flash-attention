"""Profile V1 with CUDA graphs (default mode) in-process.

Requires vLLM installed. Key trick: VLLM_ENABLE_V1_MULTIPROCESSING=0
forces V1 engine to run in the same process so torch.profiler can
capture all CUDA kernels.

Usage:
    CUDA_VISIBLE_DEVICES=0 python profile_v1_cudagraph.py
"""
import os
import sys
import time
...
MAX_TOKENS = 256
DTYPE = "float16"
OUT_PATH = "torch_trace_v1_cudagraph_bs60_dec256.json"
...
def main():
    print(f"V1 + CUDA graph profiling: {NUM_SEQS} seqs, {MAX_TOKENS} tokens")
    ...
    with profile(
        ...
        with record_function("v1_cudagraph_generate"):
            start = time.time()
            outputs = llm.generate(prompts, sp)
            elapsed = time.time() - start

    total_tokens = sum(len(o.outputs[0].token_ids) for o in outputs)
    print(f"V1+cudagraph: {total_tokens} tokens in {elapsed:.2f}s = {total_tokens/elapsed:.0f} tok/s")
