    ...
    const Params &params, const int bidb, const int bidh, const int n_block)
{
    ...
    const index_t row_offset_q = sum_s_q * params.q_row_stride
        + m_block * kBlockM * params.q_row_stride + bidh * params.q_head_stride;
    const index_t row_offset_do = sum_s_q * params.do_row_stride
        + m_block * kBlockM * params.do_row_stride + bidh * params.do_head_stride;
    const index_t row_offset_o = sum_s_q * params.o_row_stride
        + m_block * kBlockM * params.o_row_stride + bidh * params.o_head_stride;
    const index_t row_offset_dq = sum_s_q * params.dq_row_stride
        + m_block * kBlockM * params.dq_row_stride + bidh * params.dq_head_stride;
    const index_t row_offset_dq_accum = (sum_s_q * params.h * params.d_rounded)
        + (m_block * kBlockM + 128ll * bidb) * params.h * params.d_rounded + bidh * params.d_rounded
        + (!params.deterministic ? 0 : blockIdx.x * params.dq_accum_split_stride);
    const index_t row_offset_lse = bidh * params.total_q + sum_s_q + m_block * kBlockM;
    const index_t row_offset_dpsum = bidh * (params.total_q + 128 * params.b) + sum_s_q + 128 * bidb + m_block * kBlockM;
