...
inline __device__ void compute_attn_1rowblock_splitkv_dualkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {

    // dualkv attention assertion on unsupported options.
    assert(params.block_table == nullptr && "dualkv attention does not support paged attention yet.");
        ...
        for (int n_block = n_block_max - 1; n_block >= n_block_copy_min; n_block--) {
            ...
            if (params.rotary_dim == 0) {
                ...
                if (params.is_rotary_interleaved) {
                    ...
                    assert(false && "dualkv attention only supports interleaved rotary embedding.");
                    // Don't clear OOB_K because we're writing to global memory
    ...
    cute::cp_async_fence();
    //dualkv_check_tensor(tKsK, n_block);
    ...
    //                  step loop below can unroll at compile time. But in dualkv attention, we need to
    ...
    // DUALKV ATTENTION: The Padding Approach:
    ...
    if (cute::thread0()) { print(lse); printf("\ncompute_attn_1rowblock_splitkv_dualkv\n"); }
#endif
